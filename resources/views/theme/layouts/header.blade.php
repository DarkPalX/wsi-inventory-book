<style>
	...
	.dropdown-submenu .submenu {
		...
		padding-left: 1.5rem; /* indent the books */
	}
		...
		<div class="container">
			<div class="header-row justify-content-between justify-content-lg-start">
				...
				<div class="mx-lg-3 mx-sm-0 d-none d-lg-block">
					<strong class="logo-text text-uppercase" style="font-size:17px; color:#ffffff">Book Inventory System</strong>
				...
				============================================= -->
					<nav class="primary-menu with-arrows ms-lg-auto me-lg-3">
						<ul class="menu-container menu-item">
							...
							<li class="mx-1 dropdown">
								<a class="text-light menu-link" href="{{ route('books.index') }}"><i class="bi-boxes bi-alt"></i> Manage Books</a>
								<div class="dropdown-menu bg-light shadow">
									<a class="dropdown-item" href="{{ route('books.index') }}">Books List</a>
									<a class="dropdown-item" href="{{ route('books.create') }}">Create New Book</a>
								</div>
							</li>
...
@php
	$books = App\Models\Custom\Book::all();
@endphp
				...
				<div class="list-group">
					<select class="selectpicker" data-live-search="true" required>
						...
						@foreach($books as $book)
							<option value="{{ $book->id }}">{{ $book->sku }}: {{ $book->name }}</option>
						@endforeach
					</select>
				</div>

				<div class="list-group">
					{{-- <a href="{{ route('reports.stock-card', $book->id) }}" target="_blank"><button type="button" class="btn btn-success mt-2">Confirm</button></a> --}}
				</div>
			</div>
		</div>
	</div>
