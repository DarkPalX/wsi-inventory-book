                        ...
                        <div class="card-body">
							...
							<form method="post" action="{{ route('receiving.transactions.store') }}" enctype="multipart/form-data" onsubmit="return checkSelectedItems();">
								...
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<input id="book_cover" name="book_cover" class="input-file file-loading" type="file" data-show-preview="false" accept=".png, .jpg">
									</div>
								</div>
								<div class="form-group row">
									<label class="col-sm-2 col-form-label">Attachments</label>
									<div class="col-sm-10">
										<input id="attachments" name="attachments[]" class="input-file file-loading" type="file" data-show-upload="false" data-show-caption="true" data-show-preview="false" accept=".pdf" multiple>
									</div>
								</div>
								<div class="form-group row">
								...
								data.results.forEach(item => {
									resultsTableBody.append(`
										<tr>
											...
											<td><button type="button" class="btn btn-outline-primary add-item-btn" data-id="${item.id}" data-sku="${item.sku}" data-name="${item.name}" data-cost="${item.total_cost}">Add</button></td>
										</tr>
									`);
								});
							} else {
		...
		document.addEventListener('click', function(event) {
			...
			if (target.classList.contains('add-item-btn')) {
				...
				if (!exists) {
					...
					actionCell.innerHTML = '<button type="button" class="btn btn-outline-danger remove-item-btn" data-id="'+id+'" data-sku="'+sku+'" data-name="'+name+'" data-cost="'+cost+'"><i class="bi-trash"></i></button>';
