...
use \App\Models\Custom\{Book, ReceivingHeader, IssuanceHeader};
...
class FrontController extends Controller
{
    ...
    public function home()
    {
        if(Auth::user()){
            ...
            $current_receiving_count = ReceivingHeader::whereBetween('created_at', [today(), now()])->count();
            $current_issuance_count = IssuanceHeader::whereBetween('created_at', [today(), now()])->count();

            $receiving_transactions = ReceivingHeader::whereDate('updated_at', today())->orderByDesc('updated_at')->paginate($this->paginate);
            $issuance_transactions = IssuanceHeader::whereDate('updated_at', today())->orderByDesc('updated_at')->paginate($this->paginate);
            $activity_logs = ActivityLog::orderByDesc('id')->paginate(10);

            return view('theme.pages.home', compact('page', 'books_count', 'users_count', 'current_receiving_count', 'current_issuance_count', 'receiving_transactions', 'issuance_transactions', 'activity_logs'));
        }
        else{
            $page = new Page();
            $page->name = 'Login';
